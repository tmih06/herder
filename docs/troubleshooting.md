...
| `agent send` retries then fails (`agent_not_ready`) | Herdr pane exists but the agent inside hasn't come up | Check the pane via `herder task attach <id>` / `task logs`; retry once the agent binary is installed in the sandbox image |
